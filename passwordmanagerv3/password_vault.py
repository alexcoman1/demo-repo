...
def firstScreen():
    ...
    lbl = Label(window, text = "Create the master password")
...
def resetScreen():
    ...
    def getRecoveryKey():
        recoveryKeyCheck = hashPassword(str(text.get()).encode("utf-8"))
...
def loginScreen():
    ...
    def getMasterPassword():
        checkHashedPasswoord = hashPassword(txt.get().encode('utf-8'))
        global encryptionKey
        encryptionKey = base64.urlsafe_b64encode(kdf.derive(txt.get().encode()))
        cursor.execute("SELECT * FROM masterpassword WHERE id = 1 AND password = ?", [(checkHashedPasswoord)])
        print(checkHashedPasswoord)
        return cursor.fetchall()
